...
const paintStar = (function () {
    ...
    function mekeStar(
      color = "",
      colorA = "rgb(242, 201, 76)",
      colorB = "rgb(238, 237, 245)",
    ) {
      const mainSvgCode = `<path d="M6 0L7.80568 3.5147L11.7063 4.1459L8.92165 6.9493L9.52671 10.8541L6 9.072L2.47329 10.8541L3.07835 6.9493L0.293661 4.1459L4.19432 3.5147L6 0Z" `,
        pre1 = `<stop class="stp`,
        mid1 = `" offset="`,
        ps1 = `%"/>`,
        mid2 = `{stop-color:`,
        linGrd = `linearGradient`,
        figId = `ast${figNo}`,
        pre2 = `fill="`;
      let gradientDefs = "",
        ps2 = `${pre2}${color}"`,
  ...
  rateModule = (quantity = 5, rateModul, space = 20) => {
    ...
    const spaceBlocks = rateModul.querySelectorAll("." + spcBlkClasName),
      starWrapHeightD = getProp2(rateModul, "height"),
      starWrapHeight = plusPX(starWrapHeightD),
      starWrapWidthD = (starWrapHeightD * 12) / 11,
      starWrapWidth = plusPX(starWrapWidthD),
    ...
    //.ratStar = rateModul
    if (starWraps.length) {
      starWraps.forEach((starWrapI) => {
        setProp(starWrapI, { height: starWrapHeight, width: starWrapWidth });
      });
      spaceBlocks.forEach((spaceBlockI) => {
        spaceBlockI.style.width = space;
      });
    } else {
      starWraps = [];
      ...
      for (let i = 0; i < quantity; i++) {
        const starWrap = putEl(starWrapClasName, paintStar(0)),
          spcBlk = putEl(spcBlkClasName);
    ...
    function ratingDisplay(rate = 75) {
      ...
      if (rate > 99) {
        starWraps.forEach((starWrapI) => {
          starWrapI.innerHTML = paintStar(sto);
        });
        return;
      }
      /* I */
      ...
      while (rate > 0) {
        rate = rate - rateStep;
        starWraps[i].innerHTML = paintStar(sto);
        i++;
      }
      /* II */
