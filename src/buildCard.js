...
import { rateModule } from "./rateStars.js";

export const buildCard = (container, opt1 = {}, klas = {}, opt2 = {}) => {
  const opt1Default = {
      imageSrc: "goodImg0.png",
      imagePlaceHolderSrc: cnst.placeHolderSrc,
      imageAlt: "book cover image",
      author:
        "HO Commission on Social Determinants of Health, World Health Organization",
      caption:
      ...
      reviewCnt: 354,
      annot:
        "The Outrageously Funny Debut Novel About Three Super-Rich, Pedigreed Chinese Families And The Gossip about third world war",
      price: 0,
    ...
    fillDef = (defaults, inputOptions) => {
      for (let key in defaults) {
        if (!(key in inputOptions)) {
          inputOptions[key] = defaults[key];
        }
      }
    };
  fillDef(opt1Default, opt1);
  fillDef(klasDefault, klas);
  fillDef(opt2Default, opt2);
  const flx = "flex",
    sto = 100,
    blk = "block",
    none = "none",
    buyNowBtnCaption = opt1.buyNowBtnTxt.toUpperCase(),
    goodImgWrap = putEl(klas.goodImgWrap),
    goodImg = putEl(klas.goodImg, opt1.imagePlaceHolderSrc, opt1.imageAlt),
    loader = putEl(klas.loaderRing),
    coverPH = putEl(klas.coverPH, "LOADING..."),
    goodDesc = putEl(klas.goodDesc),
    rating = putEl(klas.rating),
    rateDisplay = putEl(klas.ratStar),
    price = putEl(klas.price, opt1.price),
    itemID = { author: opt1.author, caption: opt1.caption },
    innerCart = processCart(),
    rateInPercents = (opt1.averageRating * sto) / opt1.maxAverageRating,
    //set card container height 78% of width
    setPropCard = () => {
      let rateBlk;
      const cardContainerWidth = getProp2(container, "width"),
        loaderDim = cardContainerWidth / 15,
        loaderDimPX = plusPX(loaderDim),
        shd1 = plusPX(cardContainerWidth * 0.04),
        shd2 = plusPX(cardContainerWidth * 0.036),
        shd3 = plusPX(cardContainerWidth * 0.024);
      setProp(loader, {
        height: loaderDimPX,
        width: loaderDimPX,
        borderWidth: plusPX(loaderDim / 5),
      });
      setProp(container, {
        height: plusPX(cardContainerWidth / 1.277),
        fontSize: plusPX(cardContainerWidth / 54.6),
      });
      goodImg.style.boxShadow =
        plusPX() + shd1 + shd2 + "-" + shd3 + opt2.coverShdClr;
  ...
  let review = " review",
    btnClass = klas.buyNowBtn,
    buyNowBtnCaption1 = buyNowBtnCaption,
    buyNowBtn;
  //building card
  setProp(container, { display: flx, flexFlow: "row nowrap" });
  setProp(goodDesc, { display: flx, flexFlow: colNoWrap });
  ...
  goodImgWrap.appendChild(loader);
  ...
  if (opt1.price) {
    ...
    buyNowBtn.addEventListener("click", (ev) => {
      ...
      if (buyBtn.textContent === buyNowBtnCaption) {
        buyBtn.textContent = opt1.inCartTxt;
        buyBtn.classList.add("buyNowBtnPressed");
        //put item to buyer cart (Init00.js)
        processCart(itemID, true);
      } else {
        buyBtn.textContent = buyNowBtnCaption;
    ...
    buyNowBtn = putEl(klas.noBuyBtn, buyNowBtnCaption);
  }
